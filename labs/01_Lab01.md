...
> TODO:  I put this in the repo already to be automated and skipped...should you remove this or is there value?
...
6. Select the **azure-pipelines.yml** file, review the build definition
7. Navigate back to the repo, select **Set up build**
...
8. You can edit those files directly in Azure DevOps as well, and manually run them by selecting `Run` when you are ready to execute the pipeline.
...
11. The build should take about five minutes to complete.
...
12. From Azure DevOps, go to the `content-web` repository. This repository already includes `azure-pipelines.yaml`.
13. Select **Set up Build**
14. In the "Review your pipeline YAML" step, edit the `containerRegistryName` value to replace `[SHORT_SUFFIX]` with your SUFFIX so that it matches your container registry's name.
...
15. When you are finished editing, select **Save and run**.
16. In the dialog, select **Save and run**. This will start the build process.
17. Select the **Build and Push Docker Image** job, this will display the activities of the build agent.
