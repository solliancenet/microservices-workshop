...
```
TODO: how do folks find / access the build agent in this environment?
```

2. Confirm that the database contains test data.

```
TODO: should we have the user browse to cosmos? check the data?
will the machine have this access?
```
3. Navigate to the content-api directory and run the API.
...
4. Test the API using curl. You will request the speaker's content, and this will return a JSON result.
...
5. Navigate to the web application directory and run the application. 
...
6. From Azure cloud shell, run the following command to find the IP address for the build agent VM provisioned when you ran the ARM deployment.
...
7. From the cloud shell in the build machine edit the app.js file using vim.
...
8. Now run the content-web application in the background.
...
9. Test the web application using curl. You will see HTML output returned without errors.

    ```bash
    curl http://localhost:3000
    ```

10. If you received a JSON response to the /speakers content request and an HTML response from the web application, your environment is working as expected.

11. Leave the application running in the console, for the next task.
...
```
TODO: what resource group did we create? since this was done "ahead"??
```
