...
1. Switch back to the cloud shell window, ensure that you are logged out of the build machine. You may need to reconnect your shell.
...
```
TODO: not sure this flows, I pulled the ingress edits up here before deploying
seems like this would be either a) first before any deploys or b) after both deploy, but that may mess with how we test endpoints
```
1.  Now you can create an ingress resource for the content applications.

    ```bash
    TODO: we can use code on the jump?
...
2.  Create the ingress using kubectl.

    ```bash
    kubectl create --save-config=true -f content.ingress.yml
    ```

3.  Refresh the ingress endpoint in your browser. You should be able to visit the speakers and sessions pages and see all the content.

4.  Visit the api directly, by navigating to `/content-api/sessions` at the ingress endpoint.
...
7. Update your starter files by pulling the latest changes from Azure DevOps.
...
8. We will use the `helm create` command to scaffold out a chart implementation that we can build on. Use the following commands to create a new chart named `web` in a new directory:
...
9. We now need to update the generated scaffold to match our requirements. We will first update the file named `values.yaml`.
...
10. Search for the `image` definition and update the values so that they match the following:
...
11. Search for `nameOverride` and `fullnameOverride` entries and update the values so that they match the following:
